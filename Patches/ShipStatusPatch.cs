    ...
    class RepairSystemPatch
    {
            ...
            [HarmonyArgument(2)] byte amount)
        {
            ...
            if (!Options.MadmateCanFixComms.GetBool() && player.GetCustomRole().IsMadmate() //Madmateがコミュサボを直せる設定がオフ
                ...
                return false;
            if (player.Is(CustomRoles.Sheriff) || player.Is(CustomRoles.Arsonist) || (player.Is(CustomRoles.Jackal) && !Options.JackalCanUseSabotage.GetBool()) || (player.Is(CustomRoles.Samurai) && !Samurai.CanUseSabo.GetBool()))
            {
