...
| `Ctrl`+`Delete`     | 全てデフォルト値に設定       | TOH 設定画面     |
...
| -------------- | ------------------------------------- |
| /dump          | ログをダンプ                          |
| /version<br>/v | 全 MOD クライアントのバージョンを表示 |
...
定型文を設定するには AmongUs.exe と同じフォルダの`template.txt`を編集します。<br>
...
- Among Us フォルダの中に『Language』フォルダが作成されているので開きます。
...
GM はゲーム自体には何の影響も与えず、すべてのプレイヤーは誰が GM であるかがわかります。<br>
...
- ターゲットは 1 ターン/1 試合に一回設定でき、設定可能時は左向き白三角マーク(◁)が付きます。
- 変身先がインポスター、死亡済みの場合は能力は消費されません。
- 変身クールダウンはターゲット設定可能時は「5 秒」、不可時は「255 秒」で固定です。
- 変身持続時間は「1 秒」で固定されているので、変身能力自体は殆ど使えません。
...
最大 3 個の花火をシェイプシフトのタイミングで設置出来ます。<br>
...
視界が一瞬 0 になり、リアクターサボタージュ中でなければリアクターも一瞬鳴ります（キルフラッシュ）。<br>
...
死体を通報すると、通報した死体から情報を得ることが出来ます。<br>
...
自分の死体が通報されると、ランダムで一人にインポスターをチャットで表記する。<br>
特殊キルによりクルーにキルされた場合はキルした人がキル不可役職であってもインポスターと表記される。<br>
...
*インポスターを吊りきると AutoMute のミュートが解除されます。<br>
...
判定 : -<br>
カウント : -<br>
勝利条件 : 恋人が 2 人共生き残っている状態で試合が終了すること。全クルーのタスク終了時は生き残っていても敗北。<br>

全プレイヤーの中から 2 人配役されます。(他の役職に重複)<br>
...
`追加スポーン位置(エアシップ) `が OFF の場合は本来のスポーン位置からのみ選出されます。
